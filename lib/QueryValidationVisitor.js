...
module.exports = class QueryValidationVisitor {
  ...
  onArgumentEnter (arg) {
    ...
    if (isInputObjectType(argTypeDef.type)) {
      const inputObjectTypeDef = getNamedType(argTypeDef.type)
      // logDebug('onArgumentEnter inputObjectTypeDef: ', JSON.stringify(inputObjectTypeDef))
      processInputType(this.context, inputObjectTypeDef, argName, variableName, value, this.currentField)
    } else if (isListType(argTypeDef.type)) {
      ...
      if (directiveArgumentMap) {
        const fn = variableName || this.currentField.name.value + '.' + argName
        try {
          getConstraintValidateFn(getScalarType(argTypeDef.type).scalarType)(fn, directiveArgumentMap, value)
        } catch (e) {
          let error
          if (variableName) {
            error = new ValidationError(fn, `Variable "$${variableName}" got invalid value ${value}. ` + e.message, e.context)
          } else {
            error = new ValidationError(fn, `Argument "${argName}" of "${this.currentField.name.value}" got invalid value ${value}. ` + e.message, e.context)
          }
...
function processInputType (context, inputObjectTypeDef, argName, variableName, value, currentField, parentNames) {
  ...
  onInputValueDefinition (node) {
    ...
    if (value) {
      ...
      if (isInputObjectType(valueTypeDef)) {
        logDebug('InputType')
        processInputType(this.context, valueTypeDef, this.argName, this.variableName, value, this.currentField, iFieldNameFull)
      } else if (isListType(valueTypeDef)) {
        // TODO List validation
        throw new Error('List validation not implemented yet')
      } else {
        const directiveArgumentMap = getDirective(this.context.getSchema(), iFieldTypeDef, 'constraint')?.[0]
        logDebug('InputObject.onInputValueDefinition directiveArgumentMap: ', JSON.stringify(directiveArgumentMap, null, 2))
        if (directiveArgumentMap) {
          const fn = this.variableName ? this.variableName + '.' + iFieldNameFull : this.argName + '.' + iFieldNameFull
          try {
            getConstraintValidateFn(getScalarType(valueTypeDef).scalarType)(fn, directiveArgumentMap, value)
          } catch (e) {
            let error
            if (this.variableName) {
              error = new ValidationError(fn, `Variable "$${this.variableName}" got invalid value ${value} at "${this.variableName}.${iFieldNameFull}". ` + e.message, e.context)
            } else {
              error = new ValidationError(fn + '.' + iFieldNameFull, `Argument "${this.argName}" of "${this.currentField.name.value}" got invalid value ${value} at "${iFieldNameFull}". ` + e.message, e.context)
            }
            error.originalError = e
            this.context.reportError(error)
          }
