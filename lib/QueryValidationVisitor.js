      ...
      if (isInputObjectType(valueTypeDef)) {
        ...
        value.forEach(element => {
          if (isInputObjectType(valueTypeDefArray)) {
            const iFieldNameFullIndexed = `${iFieldNameFull}[${i++}]`
            validateInputTypeValue(this.context, valueTypeDefArray, this.argName, this.variableName, element, this.currentField, iFieldNameFullIndexed)
          } else {
            const fieldNameForErrorIndexed = `${fieldNameForError}[${i++}]`
            validateScalarTypeValue(this.context, this.currentField, iFieldTypeDef, valueTypeDef, element, this.variableName, this.arg, fieldNameForErrorIndexed, this.variableName ? ` at "${fieldNameForErrorIndexed}"` : ` at "${iFieldNameFull}"`)
          }
        })
      } else {
